<?php

namespace Tests\Feature\SupportTickets;

use App\Models\AgentClientAssignment;
use App\Models\SupportTicket;
use App\Models\User;
use App\Services\SupportTicketService;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Symfony\Component\HttpKernel\Exception\HttpException;
use Tests\TestCase;

class SupportTicketVisibilityTest extends TestCase
{
    use RefreshDatabase;

    public function test_agents_only_see_tickets_explicitly_assigned_to_them(): void
    {
        $service = app(SupportTicketService::class);

        $admin = User::factory()->create([
            'role_id' => User::ROLE_ADMIN,
            'status' => User::STATUS_ACTIVE,
        ]);

        $client = User::factory()->create([
            'role_id' => User::ROLE_CLIENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        $assignedAgent = User::factory()->create([
            'role_id' => User::ROLE_AGENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        $otherAgent = User::factory()->create([
            'role_id' => User::ROLE_AGENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        AgentClientAssignment::create([
            'agent_id' => $assignedAgent->id,
            'client_id' => $client->id,
            'assigned_date' => now()->toDateString(),
            'is_active' => true,
        ]);

        AgentClientAssignment::create([
            'agent_id' => $otherAgent->id,
            'client_id' => $client->id,
            'assigned_date' => now()->subDay()->toDateString(),
            'is_active' => true,
        ]);

        $assignedTicket = SupportTicket::create([
            'client_id' => $client->id,
            'agent_id' => $assignedAgent->id,
            'created_by' => $client->id,
            'subject' => 'Assigned support request',
            'status' => SupportTicket::STATUS_OPEN,
            'last_message_at' => now(),
        ]);

        $unassignedTicket = SupportTicket::create([
            'client_id' => $client->id,
            'agent_id' => null,
            'created_by' => $client->id,
            'subject' => 'Unassigned support request',
            'status' => SupportTicket::STATUS_OPEN,
            'last_message_at' => now(),
        ]);

        $this->assertSame(
            [$assignedTicket->id, $unassignedTicket->id],
            $service->queryFor($admin)->orderBy('id')->pluck('id')->all()
        );

        $this->assertSame(
            [$assignedTicket->id],
            $service->queryFor($assignedAgent)->pluck('id')->all()
        );

        $this->assertSame(
            [],
            $service->queryFor($otherAgent)->pluck('id')->all()
        );
    }

    public function test_agents_cannot_open_tickets_assigned_to_other_agents_or_unassigned_tickets(): void
    {
        $service = app(SupportTicketService::class);

        $client = User::factory()->create([
            'role_id' => User::ROLE_CLIENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        $assignedAgent = User::factory()->create([
            'role_id' => User::ROLE_AGENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        $otherAgent = User::factory()->create([
            'role_id' => User::ROLE_AGENT,
            'status' => User::STATUS_ACTIVE,
        ]);

        $assignedTicket = SupportTicket::create([
            'client_id' => $client->id,
            'agent_id' => $assignedAgent->id,
            'created_by' => $client->id,
            'subject' => 'Assigned support request',
            'status' => SupportTicket::STATUS_OPEN,
            'last_message_at' => now(),
        ]);

        $unassignedTicket = SupportTicket::create([
            'client_id' => $client->id,
            'agent_id' => null,
            'created_by' => $client->id,
            'subject' => 'Unassigned support request',
            'status' => SupportTicket::STATUS_OPEN,
            'last_message_at' => now(),
        ]);

        $service->authorize($assignedTicket, $assignedAgent);

        try {
            $service->authorize($assignedTicket, $otherAgent);
            $this->fail('Expected a 403 when another agent opens the ticket.');
        } catch (HttpException $exception) {
            $this->assertSame(403, $exception->getStatusCode());
        }

        try {
            $service->authorize($unassignedTicket, $assignedAgent);
            $this->fail('Expected a 403 when an agent opens an unassigned ticket.');
        } catch (HttpException $exception) {
            $this->assertSame(403, $exception->getStatusCode());
        }
    }
}
